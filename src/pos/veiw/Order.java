...
import pos.listener.OrderEditListenerTemplet;
import pos.listener.OrderListener;
import pos.listener.OrderListenerTemplet;
import pos.veiw.OrderTemplet;
...
public class Order extends JPanel {
	...
	public OrderTemplet[] menuPanelarr;
	public int count;
	public Vector<MenuDTO> menu;
	public String restId;
	public Vector<String> menuType;
	OrderEditListenerTemplet menulitener;
	/**
	 * Create the panel.
	 */
	public Order(MainFrame frame) {
		this.frame = frame;
		restId=frame.ID;
		...
		menuPanelarr=new OrderTemplet[count];
		for(int i=0;i<menuPanelarr.length;i++){
			menuPanelarr[i]=new OrderTemplet(this, menuType.get(i));   
		...
		custoOdertable = new JTable(model);
		custoOdertable.setEnabled(true);
		custoOdertable.getColumnModel().getColumn(0).setPreferredWidth(160);
		custoOdertable.getColumnModel().getColumn(1).setPreferredWidth(62);
		custoOdertable.getColumnModel().getColumn(2).setPreferredWidth(97);
		custoOdertable.setRowHeight(30);
	    
		
		
		scrollPane.setViewportView(custoOdertable);
		...
		btnOrder.setFont(new Font("±¼¸²", Font.BOLD, 23));
		btnOrder.setFocusPainted(false);
		btnOrder.setBackground(new Color(243, 50, 11));
		btnOrder.setBounds(696, 446, 111, 89);
		add(btnOrder);
	...
	private void setEvent() {
		OrderListener listener = new OrderListener(this);
		btnOrder.addActionListener(listener);
		eatdaButton_1.addActionListener(listener);
		menulitener=new OrderEditListenerTemplet(this);
	      for(int i=0;i<menuPanelarr.length;i++){
